...
if input_mode == "Elevation-Based":
    ...
    evc_station = st.number_input("EVC Station", value=0.0, step=1.0, format="%.2f")
    evc_elevation = st.number_input("EVC Elevation", value=0.0, step=0.01)

    default_pvi_station = (bvc_station + evc_station) / 2 if (bvc_station and evc_station) else 0.0
    pvi_station = st.number_input("PVI Station", value=default_pvi_station, step=1.0, format="%.2f")
    pvi_elevation = st.number_input("PVI Elevation", value=0.0, step=0.01)
    ...
    evc_station = st.number_input("EVC Station", value=0.0, step=1.0, format="%.2f")
    bvc_elevation = st.number_input("BVC Elevation", value=0.0, step=0.01)
    g1 = st.number_input("Grade In (g₁) [%]", value=0.0, step=0.01, format="%.2f")
    g2 = st.number_input("Grade Out (g₂) [%]", value=0.0, step=0.01, format="%.2f")
...
if use_custom_k:
    k_value = st.number_input("K-value", value=0.0, step=0.01)
else:
...
station_input = st.number_input("Enter Station", value=0.0, step=1.0, format="%.2f")
